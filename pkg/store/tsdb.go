...
func NewTSDBStore(logger log.Logger, _ prometheus.Registerer, db TSDBReader, component component.StoreAPI, externalLabels labels.Labels) *TSDBStore {
	...
	return &TSDBStore{
		...
		// TODO(bwplotka): Consider 1MB as recommended by gRPC. Initial benchmarks not involving
		// networking and marshalling suggests no framing being a better option.
		maxBytesPerFrame: math.MaxInt64,
	}
}
